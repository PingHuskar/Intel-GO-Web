  ...
  BTS: {
    ...
    OTHERINFO: {
      ...
      App: {
        ...
        Android:
          "https://play.google.com/store/apps/details?id=com.bts.skytrain",
      },
  ...
  MRT: {
    ...
    OTHERINFO: {
      ...
      App: {
        ...
        Android:
          "https://play.google.com/store/apps/details?id=com.devsenses.mrt_app&hl=th",
      },
...
let zoom = localStorage.getItem(`zoom`) || 11;
var map,
  mrkCurrentLocation,
  popExample,
  ctlZoom,
    ...
    exits: [
      {
        ...
        places: ["สยามเซ็นเตอร์", "สยาม ดิส", "วังสระปทุม"],
      },
      {
        latlng: {
          lat: 13.745613,
...
const printFare = (faredata, stationType) => {
  if (stationType === `BRT`) {
    for (let [custType, fare] of Object.entries(faredata)) {
      console.log(
        `${custType.padEnd(33)}: ${fare.toString().padStart(3)} ${
          DICT.BAHT[lang]
        }`
      );
    }
    ...
    for (let [custType, numberOfStationAndFare] of Object.entries(faredata)) {
      ...
      for (let [numberofstation, fare] of Object.entries(
        numberOfStationAndFare
      )) {
        console.log(
          `${numberofstation.toString().padStart(2)} ${
            DICT.STOPS[lang]
          } : ${fare.toString().padStart(3)} ${DICT.BAHT[lang]}`
        );
      }
    }
...
const printStationDetail = (station) => {
  ...
  try {
    printStationAge(
      station.type,
      station.en ?? station.name ?? station.AKA,
      station.since
    );
  } catch {}
  try {
...
const getStationImage = (station) => {
  if (["BTS", "MRT"].includes(station.type)) {
    return `<a href="../src/images/${station.type}/${station.id.replace(
      /\-.*/,
      ""
    )}.${stationImageExtension[station.type]}" target="_blank">
        <img alt="${station.id}" src="../src/images/${
      station.type
    }/${station.id.replace(/\-.*/, "")}.${
      stationImageExtension[station.type]
...
const getBannergressBanner = (station) => {
  ...
  return `<a href="https://bannergress.com/banner/${
    station.bannergress[0].path
  }" target="_blank">
    <img 
    alt="${station.bannergress[0].path
      .slice(0, station.bannergress[0].path.length - 5)
      .replace(/\-/g, " ")
      .replace(/(^\w{1})|(\s+\w{1})/g, (letter) => letter.toUpperCase())}" 
    title="${station.bannergress[0].path
      .slice(0, station.bannergress[0].path.length - 5)
      .replace(/\-/g, " ")
      .replace(/(^\w{1})|(\s+\w{1})/g, (letter) => letter.toUpperCase())}" 
    src="https://api.bannergress.com/bnrs/pictures/${
      station.bannergress[0].bg
    }" style="width:${station.bannergress[0].w}px;height:${
    station.bannergress[0].h
  }px;">
</a>`;
};
...
for (let station of STATIONS) {
  ...
  if (station.hasOwnProperty("exits")) {
    for (let [i, exit] of station.exits.entries()) {
      L.marker(
        { lat: exit.latlng.lat, lng: exit.latlng.lng },
        {
          icon: L.divIcon({
        ...
        .bindPopup(
          `
                <h6><b>${station.name}</b> ทางออก <b>${
            i + 1
          }</b> สถานที่ใกล้เคียง</h6>
                <ul>
