...
for (let record of sites) {
    if (record.geo.length > 0) {
        const NAME = record.name
        markers.addLayer(L.marker(new L.LatLng(...record.geo))
        .bindPopup(`<h2>${NAME}</h2>
        <p>ต.${record.subdistrict} อ.${record.district} จ.${record.province}</p>
        <p>${record.importance}</p>
        ...
        AddDonut(record.geo.at(0), record.geo.at(1), 10000)
    }
}
